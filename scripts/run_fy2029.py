...
# プロジェクトルート（scripts/ の親）から src/ をインポートパスに追加
...
PRODUCTS_CSV = _ROOT / "data" / "input" / "products.csv"
...
def load_data():
    ...
    _mr_monthly_counts = (
        _act_for_mr.groupby(["product_id", "activity_ym"])["activity_count"]
        .sum()
...
def main():
    ...
    comp_path = _ROOT / "data" / "input" / "competitor_schedule.csv"
    if comp_path.exists():
    ...
    if not _act.empty and "product_id" in _act.columns:
        ...
        if "activity_ym" in _act_ym.columns:
            _mr_monthly = (
                _act_ym.groupby(["product_id", "activity_ym"])["activity_count"]
                .sum()
                .reset_index(name="count")
                .groupby("product_id")["count"]
                .mean()
    ...
    calculator = FY2029FTECalculator(
        ...
        competition_count_factor=0.05,  # 競合品 1 品発売済みにつき 5% FTE 増（0.0 で無効化）
    ...
    # raw_summary_fy: required_fte（活動計算の生値）を使用
    # adjusted_fte は run_with_dynamic_new_product により既にFTE移転が適用され
    # 領域合計がほぼヘッドカウント目標値になっているため使わない
    _raw_for_area = raw_fte_df.drop(
        columns=["adjusted_fte", "adjusted_mr_fte", "fte_reduction"], errors="ignore"
    )
    raw_summary_fy = calculator.total_fte_by_area_fy(_raw_for_area)
    # monthly版も保持（fig_fte_vs_headcount で正規化前の真の必要FTEを表示するため）
    raw_total_fte_monthly = calculator.total_fte_by_area(_raw_for_area)
    ...
    # 年度ヘッドカウント目標に正規化
    # キャップ不要月（合計 <= ヘッドカウント目標）→ required_fte そのまま
    # キャップあり月（合計 > ヘッドカウント目標）→ Hill関数等限界収益で最適配分
    #   MMMパラメータあり品目: 限界収益の高い品目を優先保護（飽和品目を先に削減）
    #   MMMパラメータなし品目: グループ内比例スケーリング
    fte_df = normalize_fte_roi_weighted(
        fte_df, CURRENT_MR_COUNT, decay_params_df=data["decay_params_df"]
    )

    # 月次FTEを半期（6ヶ月）ステップ関数に変換（離散的な計画値）
    # decay_params_df を渡すことで再正規化もROI方式を使用
    fte_df = discretize_fte_semiannually(
        fte_df, CURRENT_MR_COUNT, decay_params_df=data["decay_params_df"]
    )
    ...
    summary_df    = calculator.summarize_fy(fte_df)           # キャップあり（ROI配分後）
    raw_summary_df = calculator.summarize_fy(raw_fte_df)      # キャップなし（活動積み上げ理論値）
    # total_fte_df: 正規化前の required_fte を使用（正規化後は必要FTE≈現行MR数になり比較が無意味）
    total_fte_df  = raw_total_fte_monthly
    total_fte_fy  = calculator.total_fte_by_area_fy(fte_df)   # キャップあり・領域別年度集計
    ...
    reporter.generate(
        ...
        raw_fte_df=raw_fte_df,
        raw_summary_df=raw_summary_df,   # キャップなし品目別積み上げ用
        total_fte_fy=total_fte_fy,       # キャップあり領域別年度集計用
