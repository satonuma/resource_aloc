...
def fig_fte_by_product_bar(summary_df: pd.DataFrame, target_fy: str = "") -> go.Figure:
    """
    品目別 平均必要FTE（MRのみ）の横棒グラフ（FTE降順）。
    summary_df: summarize_fy() の出力
    """
    # FTE昇順ソート（横棒では上が最大になる）
    df = summary_df.copy().sort_values("avg_required_fte", ascending=True)
    fy_label = f" - {target_fy}" if target_fy else ""

    fig = go.Figure()
    fig.add_trace(go.Bar(
        name="必要FTE（MR）",
        y=df["product_id"],           # 製品名を Y 軸（縦リスト）
        x=df["avg_required_fte"],     # FTE 値を X 軸（横棒）
        orientation="h",
...
def fig_monthly_fte_trend(fte_df: pd.DataFrame) -> go.Figure:
    """
    品目×月別 FTE 推移: 領域別（CS / PS遺伝 / PS血液）の 3 行 × 2 列サブプロット。
    左列=絶対値ライン、右列=100%積み上げエリア（各領域内の構成比）。
    """
    ...
    fig = make_subplots(
        rows=3, cols=2,
        subplot_titles=[
            "CS 品目別月次FTE（積み上げ）",    "CS 品目別月次FTE構成比",
            "PS遺伝 品目別月次FTE（積み上げ）", "PS遺伝 品目別月次FTE構成比",
            "PS血液 品目別月次FTE（積み上げ）", "PS血液 品目別月次FTE構成比",
        ],
    ...
    for row_idx, area in enumerate(areas, start=1):
        area_df = fte_df[fte_df["area"] == area].copy()
        if area_df.empty:
            continue

        products = sorted(area_df["product_id"].unique())

        # 月×品目のピボット（100%積み上げ用）
        pivot = (
            area_df.groupby(["month", "product_id"])[fte_col]
            .sum()
        ...
        for pid in products:
            sub = area_df[area_df["product_id"] == pid].sort_values("month")
            color = PRODUCT_COLORS.get(pid, "#aaa")
            has_fte = bool(sub[fte_col].sum() > 0)

            # 左: 絶対値 積み上げエリア（stackgroup でスタック）
            # 1品目が支配的になっても全品目の貢献が可視化される
            fig.add_trace(
                go.Scatter(
                    name=pid,
                    x=sub["month"].tolist(),
                    y=sub[fte_col].round(2).tolist(),
                    mode="lines",
                    stackgroup=f"abs_{area}",
                    fillcolor=color,
                    line=dict(color=color, width=0.5),
                    showlegend=has_fte,
                    legendgroup=pid,
                    legendgrouptitle_text=area if pid == products[0] else None,
                    hovertemplate=f"{pid}: %{{y:.1f}} FTE<extra></extra>",
                ),
                row=row_idx, col=1,
            )
    ...
    fig.update_layout(
        height=900,
        template="plotly_white",
        title="品目別月次FTE推移（cap あり・領域別）",
        legend=dict(orientation="v", y=1.0, x=1.02, font=dict(size=10), tracegroupgap=12),
    )
    fig.update_xaxes(tickangle=45)
    for row_idx, area in enumerate(areas, start=1):
        fig.update_yaxes(title_text="FTE（人）積み上げ", row=row_idx, col=1)
        fig.update_yaxes(title_text="構成比（%）", range=[0, 100], row=row_idx, col=2)
    return fig


def fig_cap_comparison(
    raw_fte_df: pd.DataFrame,
    cap_fte_df: pd.DataFrame,
) -> go.Figure:
    """
    cap なし（理想FTE・制約前）vs cap あり（ヘッドカウント正規化後）の比較チャート。

    上段: 領域別 合計FTE 推移（3 列: CS / PS遺伝 / PS血液）
      - 実線: cap なし 合計FTE（活動積み上げ本来値）
      - 点線: cap あり 合計FTE（= 現行ヘッドカウント目標、常に水平）
      - 赤破線: 現行MR数（参照線）
    下段: 品目別積み上げ面グラフ（cap なし / cap あり の構成比較）
    """
    from fy2029_fte_calculator import CURRENT_MR_COUNT

    raw_col = "adjusted_fte" if "adjusted_fte" in raw_fte_df.columns else "required_fte"
    cap_col = "adjusted_fte" if "adjusted_fte" in cap_fte_df.columns else "required_fte"
    ...
    fig = make_subplots(
        rows=2, cols=3,
        subplot_titles=[
            "CS 合計FTE推移", "PS遺伝 合計FTE推移", "PS血液 合計FTE推移",
            "CS 品目構成（cap なし）", "PS遺伝 品目構成（cap なし）", "PS血液 品目構成（cap なし）",
        ],
    ...
    for col_idx, area in enumerate(areas, start=1):
        headcount = CURRENT_MR_COUNT.get(area, 0)
        color = area_colors[area]

        # cap なし: raw 合計 per month
        raw_area = raw_fte_df[raw_fte_df["area"] == area]
        raw_total = (
            raw_area.groupby("month")[raw_col].sum()
            .reset_index(name="total_fte")
            .sort_values("month")
        )

        # cap あり: normalized 合計 per month（ = 目標ヘッドカウントで水平）
        cap_area = cap_fte_df[cap_fte_df["area"] == area]
        cap_total = (
            cap_area.groupby("month")[cap_col].sum()
            .reset_index(name="total_fte")
            .sort_values("month")
        )
        ...
        months = raw_total["month"].tolist()

        # cap なし 実線
        fig.add_trace(go.Scatter(
            name=f"{area} cap なし（理想）",
            x=months,
            y=raw_total["total_fte"].round(1).tolist(),
            mode="lines",
            line=dict(color=color, width=2.5),
            legendgroup=f"{area}_raw",
            showlegend=True,
        ), row=1, col=col_idx)

        # 現行ヘッドカウント 参照線（赤破線）
        fig.add_trace(go.Scatter(
            name=f"現行MR数: {headcount}名",
            x=months,
            y=[headcount] * len(months),
            mode="lines",
            line=dict(color="#d62728", width=2, dash="dash"),
            legendgroup=f"headcount_{area}",
            showlegend=True,
        ), row=1, col=col_idx)

        # cap あり 点線（= 目標ヘッドカウントと一致するが explicit に表示）
        fig.add_trace(go.Scatter(
            name=f"{area} cap あり（配分後）",
            x=cap_total["month"].tolist(),
            y=cap_total["total_fte"].round(1).tolist(),
            mode="lines",
            line=dict(color=color, width=2, dash="dot"),
            legendgroup=f"{area}_cap",
            showlegend=True,
        ), row=1, col=col_idx)

        # 充足率アノテーション（cap なし 平均 vs headcount）
        avg_raw = raw_total["total_fte"].mean()
        fill_rate = headcount / avg_raw * 100 if avg_raw > 0 else 100.0
        shortage = avg_raw - headcount
        sign = "不足" if shortage > 0 else "余剰"
        fig.add_annotation(
            text=f"平均充足率 {fill_rate:.0f}%<br>（{sign} {abs(shortage):.0f}人）",
        ...
        # ---- 下段: 品目別積み上げ（cap なし）----
        products = sorted(raw_area["product_id"].unique())
        pivot_raw = (
            raw_area.groupby(["month", "product_id"])[raw_col]
            .sum()
            .unstack(fill_value=0)
            .reindex(columns=products, fill_value=0)
        )
        for pid in products:
            pid_color = PRODUCT_COLORS.get(pid, "#aaa")
            has_data = bool(pivot_raw[pid].sum() > 0) if pid in pivot_raw.columns else False
            if not has_data:
                continue
            fig.add_trace(go.Scatter(
                name=pid,
                x=pivot_raw.index.tolist(),
                y=pivot_raw[pid].round(1).tolist(),
                mode="lines",
                stackgroup=f"raw_{area}",
                fillcolor=pid_color,
                line=dict(color=pid_color, width=0.5),
                showlegend=False,
                legendgroup=pid,
            ), row=2, col=col_idx)
    ...
    fig.update_layout(
        ...
        template="plotly_white",
        title=(
            "合計FTE シミュレーション比較: cap なし（理想）vs cap あり（制約後）<br>"
            "<sup>上段: 領域別合計FTE推移 ／ 下段: cap なし の品目別積み上げ</sup>"
        ),
        legend=dict(
            orientation="h",
            y=-0.14,
    ...
    fig.update_xaxes(tickangle=45)
    for col_idx, area in enumerate(areas, start=1):
        fig.update_yaxes(title_text="FTE（人）", row=1, col=col_idx)
        fig.update_yaxes(title_text="FTE（人）", row=2, col=col_idx)
    return fig
...
def fig_digital_activity(digital_act_df: pd.DataFrame) -> go.Figure:
    ...
    for act_type, color in [("webinar", "#2563eb"), ("e_contents", "#16a34a")]:
        sub = (
            ...
            .set_index("product_id")[["view_count"]]
...
def fig_digital_trend(digital_act_df: pd.DataFrame) -> go.Figure:
    ...
    activity_date は "YYYY/M/D"（月日が1桁の場合あり）形式。
    str[:7] では "2024/4/" のように末尾スラッシュが付きソートが狂うため
    pd.to_datetime で正しく YYYY-MM 形式に変換する。
    """
    if digital_act_df.empty:
        return go.Figure()
    ...
    # 日付を YYYY-MM 形式に正規化（"2024/4/9" → "2024-04" など）
    if "activity_ym" not in df.columns and "activity_date" in df.columns:
        df["activity_ym"] = (
            pd.to_datetime(df["activity_date"], format="%Y/%m/%d", errors="coerce")
            .dt.strftime("%Y-%m")
        )
    ...
    # NaT を除去
    df = df.dropna(subset=["activity_ym"])
    ...
    for col_idx, act_type in enumerate(["webinar", "e_contents"], start=1):
        ...
        monthly = (
            ...
            .size()
            .reset_index(name="view_count")
            .sort_values("activity_ym")   # YYYY-MM 形式なので辞書順＝時系列順
        )
        # 全期間の月一覧（全品目共通 x 軸）
        all_months = sorted(monthly["activity_ym"].unique())

        for pid in sorted(monthly["product_id"].unique()):
            s = monthly[monthly["product_id"] == pid].sort_values("activity_ym")
            # 月が抜けている場合は NaN で補間して連続表示
            s_full = (
                pd.DataFrame({"activity_ym": all_months})
                .merge(s, on="activity_ym", how="left")
            )
            color = PRODUCT_COLORS.get(pid, "#aaa")
            fig.add_trace(go.Scatter(
                name=pid,
                x=s_full["activity_ym"].tolist(),
                y=s_full["view_count"].tolist(),
                mode="lines",
                line=dict(color=color, width=1.5),
                connectgaps=True,
                showlegend=(col_idx == 1),
                legendgroup=pid,
                hovertemplate=f"{pid}: %{{y:.0f}}件<br>%{{x}}<extra></extra>",
            ), row=1, col=col_idx)
...
def fig_raw_fte_by_fy(raw_summary_fy: pd.DataFrame) -> go.Figure:
    ...
    NOTE: Scatter に mode="lines+text" + textposition を使うとサブプロットの
    Y軸スケールが狂う場合があるため、mode="lines" のみ使用する。
    """
    from fy2029_fte_calculator import CURRENT_MR_COUNT

    if raw_summary_fy is None or raw_summary_fy.empty:
        return go.Figure()

    fig = make_subplots(
        rows=1, cols=3,
        subplot_titles=(
            "CS領域 本来必要FTE vs 現行MR数",
            "PS遺伝領域 本来必要FTE vs 現行MR数",
            "PS血液領域 本来必要FTE vs 現行MR数",
        ),
        horizontal_spacing=0.10,
    )
    ...
    for area in ["CS", "PS遺伝", "PS血液"]:
        ...
        # ★ .tolist() で Python list に変換（pandas Series のまま渡すと bdata 化して
        #   y軸オートスケールが scatter 値のみで計算されバーがクリップされる）
        fy_list  = sub["fiscal_year"].tolist()
        fte_list = [round(float(v)) for v in sub["avg_total_fte"]]
        y_max    = max(fte_list + [current]) * 1.25
        ...
        # y軸範囲を明示的に設定
        fig.update_yaxes(range=[0, y_max], row=1, col=col)

    fy_vals_all = sorted(raw_summary_fy["fiscal_year"].unique()) if "fiscal_year" in raw_summary_fy.columns else []
    fy_range_label = f"（{fy_vals_all[0]}〜{fy_vals_all[-1]}）" if fy_vals_all else ""

    fig.update_layout(
        height=440,
        template="plotly_white",
        title=f"本来必要FTE（活動から積み上げ・正規化前）vs 現行MR数{fy_range_label}",
...
def fig_fte_vs_headcount(total_fte_df: pd.DataFrame) -> go.Figure:
    """
    領域別 本来必要FTE（正規化前）vs 現行MR数の月別比較。

    total_fte_df は正規化前の required_fte を使った total_fte_by_area() の出力を渡すこと。
    正規化後の値を渡すと必要FTE ≈ 現行MR数 となりギャップが見えなくなる。

    ギャップの色分け:
      赤 = 不足（必要FTE > 現行MR数）→ 供給不足
      緑 = 余剰（必要FTE < 現行MR数）→ 余剰リソース
    """
    from fy2029_fte_calculator import CURRENT_MR_COUNT
    ...
    for col_idx, area in enumerate(["CS", "PS遺伝", "PS血液"], start=1):
        ...
        fte_vals = sub["total_fte"].round(1).tolist()
        mr_vals  = [current] * len(months)

        # ---- 不足ゾーン（必要FTE > 現行MR数）: 赤塗り ----
        shortage_y_top = [max(f, c) for f, c in zip(fte_vals, mr_vals)]
        shortage_y_bot = [min(f, c) for f, c in zip(fte_vals, mr_vals)]
        shortage_mask  = [f > c for f, c in zip(fte_vals, mr_vals)]
        # toself fill: 時系列順 → 逆順でbottomを繋ぐ
        xs_gap  = months + months[::-1]
        ys_short = [t if m else c for t, c, m in zip(shortage_y_top, mr_vals, shortage_mask)] + \
                   [b if m else c for b, c, m in zip(shortage_y_bot, mr_vals, shortage_mask)][::-1]
        ...
        fig.add_trace(go.Scatter(
            ...
            showlegend=(col_idx == 1),
            legendgroup="mr",
            legendgrouptitle_text="基準" if col_idx == 1 else None,
        ), row=1, col=col_idx)
        ...
        fig.add_trace(go.Scatter(
            ...
            fill="tonexty",
            fillcolor=(
                "rgba(214,39,40,0.15)"   # 赤: 不足ゾーン
                if any(f > c for f, c in zip(fte_vals, mr_vals))
                else "rgba(44,160,44,0.12)"  # 緑: 余剰ゾーン
            ),
            hovertemplate=f"{area}<br>月: %{{x}}<br>必要FTE: %{{y:.1f}}<extra></extra>",
            showlegend=True,
        ), row=1, col=col_idx)
    ...
    fig.update_layout(
        title=f"本来必要FTE（正規化前）vs 現行MR数{fy_range_label}",
        height=430,
        template="plotly_white",
...
def fig_fy_trend(summary_df: pd.DataFrame, title_suffix: str = "") -> go.Figure:
    """
    品目×年度の平均FTE推移（グループ別積み上げ棒グラフ）
    summary_df: summarize_fy() の出力（fiscal_year カラム必須）
    title_suffix: タイトルに追加するラベル（例: "キャップあり" / "キャップなし"）
    """
    ...
    for pid in products:
        ...
        y_vals = [float(sub.loc[fy, "avg_required_fte"]) if fy in sub.index else 0.0 for fy in fy_list]
        fig.add_trace(go.Bar(
            name=pid,
            x=fy_list,
            y=y_vals,
    ...
    fig.update_layout(
        barmode="stack",
        title=f"品目別 年度FTE推移{suffix_label}（{fy_range}）",
        xaxis_title="年度",
        yaxis_title="平均FTE（人）",
        height=560,
        template="plotly_white",
...
def fig_fy_area_capped(total_fte_fy: pd.DataFrame) -> go.Figure:
    """
    領域×年度の配分済みFTE（ROI最適配分後・キャップあり）vs 現行MR数。

    キャップあり月（合計 > ヘッドカウント目標）は ROI最適配分で合計 = MR目標になる。
    キャップ不要月（合計 <= 目標）は必要FTEをそのまま反映し MR目標を下回る。

    total_fte_fy: total_fte_by_area_fy(normalized_fte_df) の出力
    """
    from fy2029_fte_calculator import CURRENT_MR_COUNT

    if total_fte_fy is None or total_fte_fy.empty:
        return go.Figure()

    fy_vals = sorted(total_fte_fy["fiscal_year"].unique()) if "fiscal_year" in total_fte_fy.columns else []
    area_fy_range = f"（{fy_vals[0]}〜{fy_vals[-1]}）" if fy_vals else ""
    ...
    for col_idx, area in enumerate(["CS", "PS遺伝", "PS血液"], start=1):
        sub = total_fte_fy[total_fte_fy["area"] == area].sort_values("fiscal_year")
        if sub.empty:
            continue
        current = CURRENT_MR_COUNT.get(area, 0)
        color = area_colors[area]

        x_list   = sub["fiscal_year"].tolist()
        fte_list = [round(float(v)) for v in sub["avg_total_fte"]]
        y_max    = max(fte_list + [current]) * 1.25
        ...
        fig.add_trace(go.Scatter(
            name=f"{area} 現行MR数 ({current}名)",
            x=x_list,
            y=[float(current)] * len(x_list),
            mode="lines",
            line=dict(color="#d62728", dash="dash", width=2.5),
            showlegend=True,
        ), row=1, col=col_idx)

        fig.update_yaxes(range=[0, y_max], row=1, col=col_idx)

    fig.update_layout(
        title=f"領域別 配分済みFTE（ROI最適配分後）vs 現行MR数{area_fy_range}",
        height=460,
        template="plotly_white",
...
def fig_fy_trend_area(raw_summary_fy: pd.DataFrame) -> go.Figure:
    """
    領域×年度の本来必要FTE（正規化前）推移 + 現行MR数比較。

    正規化後の合計FTEは常にヘッドカウント目標と一致するため意味がない。
    raw_summary_fy（正規化前）の本来必要FTEを使い、実際の需要ギャップを示す。
    raw_summary_fy: total_fte_by_area_fy(raw_fte_df) の出力（正規化前）

    NOTE: go.Bar に pandas Series を渡すと Plotly が binary bdata 形式で格納し、
    y軸オートスケールが scatter trace の値（現行MR数）のみで計算されてバーが
    クリップされる問題が発生する。必ず .tolist() で Python list に変換して渡すこと。
    また textposition="outside" は y軸上限を超えるバーで軸スケールを壊すため
    "inside" を使用する。
    """
    from fy2029_fte_calculator import CURRENT_MR_COUNT
    ...
    fy_vals = sorted(raw_summary_fy["fiscal_year"].unique()) if "fiscal_year" in raw_summary_fy.columns else []
    area_fy_range = f"（{fy_vals[0]}〜{fy_vals[-1]}）" if fy_vals else ""

    fig = make_subplots(
        rows=1, cols=3,
        subplot_titles=("CS領域 本来必要FTE", "PS遺伝領域 本来必要FTE", "PS血液領域 本来必要FTE"),
        horizontal_spacing=0.10,
    )
    area_colors = {"CS": "#1f77b4", "PS遺伝": "#ff7f0e", "PS血液": "#2ca02c"}
    ...
    for col_idx, area in enumerate(["CS", "PS遺伝", "PS血液"], start=1):
        ...
        x_list   = sub["fiscal_year"].tolist()
        fte_list = [round(float(v)) for v in sub["avg_total_fte"]]
        y_max    = max(fte_list + [current]) * 1.25  # y軸上限を明示的に設定

        # 本来必要FTE バー（正規化前の活動積み上げ値）
        fig.add_trace(go.Bar(
            name=f"{area} 本来必要FTE",
            x=x_list,
            y=fte_list,
            ...
            opacity=0.75,
            text=fte_list,
            textposition="inside",   # outside は y軸上限超えバーで軸スケールを破壊
            textfont=dict(color="white", size=10),
            showlegend=True,
        ), row=1, col=col_idx)
        ...
        # y軸範囲を明示的に設定（y軸オートスケール問題を回避）
        fig.update_yaxes(range=[0, y_max], row=1, col=col_idx)

    fig.update_layout(
        title=f"領域別 本来必要FTE（活動積み上げ・正規化前）vs 現行MR数{area_fy_range}",
        height=460,
        template="plotly_white",
        legend=dict(orientation="h", y=-0.20, x=0.5, xanchor="center"),
        margin=dict(b=100),
...
def fig_sim_vs_optimal(
    ...
    """
    シミュレーションFTE vs ROI最適FTEの品目別横棒比較チャート（FY2029）。

    左パネル: シミュレーション FTE（現在の配分）vs ROI最適 FTE（等限界収益配分）横棒
    右パネル: FTE差分（sim − opt）= 正: 過剰配分、負: 過少配分
    """
    TARGET_FY = "FY2029"

    if optimal_fte_df.empty or "fiscal_year" not in summary_df.columns:
        return go.Figure()
    ...
    # FTE>0の品目に絞る（昇順ソートで横棒の上が最大になる）
    products = sorted(
        [p for p in sim_fy["product_id"].unique()
         if sim_fy.loc[sim_fy["product_id"] == p, "avg_required_fte"].sum() > 0],
        key=lambda p: sim_fy.loc[sim_fy["product_id"] == p, "avg_required_fte"].sum(),
        reverse=False,  # 横棒: 昇順 = 上が最大
    )

    sim_vals = [sim_fy.loc[sim_fy["product_id"] == p, "avg_required_fte"].sum() for p in products]
    opt_vals = [opt_vals_map.get(p, 0.0) for p in products]
    diff_vals = [s - o for s, o in zip(sim_vals, opt_vals)]

    fig = make_subplots(
        rows=1, cols=2,
        subplot_titles=(
            f"品目別 シミュレーション vs ROI最適FTE（{TARGET_FY}）",
            "FTE差分（シミュレーション − ROI最適）",
        ),
        horizontal_spacing=0.12,
    )
    ...
    # ---- 右: 差分（sim - opt）横棒 ----
    diff_colors = ["#d62728" if d > 0 else "#2ca02c" for d in diff_vals]
    fig.add_trace(go.Bar(
        name="差分 (sim − opt)",
        y=products,
        x=diff_vals,
        ...
        text=[f"{d:+.1f}" for d in diff_vals],
        textposition="outside",
        showlegend=False,
    ), row=1, col=2)

    # 差分 0 ライン
    fig.add_vline(x=0, line_dash="solid", line_color="#333", line_width=1.5, row=1, col=2)

    fig.update_layout(
        barmode="group",
        height=500,
        template="plotly_white",
        title="シミュレーションFTE vs ROI最大化最適FTE（FY2029）",
        legend=dict(orientation="h", y=-0.14, x=0.5, xanchor="center"),
        margin=dict(b=80, l=80),
    )
    fig.update_xaxes(title_text="FTE（人）", row=1, col=1)
    fig.update_xaxes(title_text="差分 FTE（人）", row=1, col=2)
    return fig
...
def fig_fc_sc_breakdown(fte_df: pd.DataFrame) -> go.Figure:
    ...
    FY2029 の実際の fc_ratio を使用（全年度平均ではなく目標年度の実態）。
    全年度平均だと「FY2027-2028のみSC化」品目が恒久的にSCに見えてしまうため。

    色分け:
      濃青 = FC専用（fc_ratio=1.0）
      中青 = 主FC（0.5 < fc_ratio < 1.0）
      橙   = 主SC（fc_ratio ≤ 0.5）
      赤   = SC専用（fc_ratio=0.0）
    """
    ...
    # FY2029 の値に限定（目標年度の実態を表示）
    fy_col = None
    if "fiscal_year" in fte_df.columns:
        fy_col = "fiscal_year"
    else:
        # month列からFY判定
        tmp = fte_df.copy()
        tmp["_fy"] = tmp["month"].apply(
            lambda m: f"FY{int(m[:4])}" if int(m[5:7]) >= 4 else f"FY{int(m[:4])-1}"
        )
        fy_col = "_fy"
        fte_df = tmp

    fy_target = "FY2029"
    fy_avail = fte_df[fy_col].unique()
    if fy_target not in fy_avail:
        fy_target = sorted(fy_avail)[-1]

    sub = fte_df[fte_df[fy_col] == fy_target]
    avg = (
        sub.groupby("product_id")[["fc_ratio", "fc_weight"]]
        .mean()
        .reset_index()
        .sort_values("fc_ratio", ascending=True)   # 横棒: 昇順 = 上が最大
    )

    # 色分け
    def _bar_color(r: float) -> str:
        if r == 0.0:   return "#d62728"   # SC専用: 赤
        if r <= 0.5:   return "#ff7f0e"   # 主SC: 橙
        if r < 1.0:    return "#6baed6"   # 主FC: 中青
        return "#1f77b4"                  # FC専用: 濃青

    colors = [_bar_color(r) for r in avg["fc_ratio"]]
    ...
    fig.add_trace(go.Bar(
        name="FC比率（主訪問割合）",
        y=avg["product_id"],
        x=(avg["fc_ratio"] * 100).round(1),
        orientation="h",
        marker_color=colors,
        text=(avg["fc_ratio"] * 100).round(0).astype(int).astype(str) + "%",
        textposition="outside",
        hovertemplate="%{y}: FC比率=%{x:.0f}%<extra></extra>",
    ))

    # 参照線: FC=100%（基準）
    fig.add_vline(x=100, line_dash="dot", line_color="#999", line_width=1)

    # fc_weight をアノテーションとして散布点で重ね表示
    fig.add_trace(go.Scatter(
        name="訪問コスト重み（fc_weight）",
        y=avg["product_id"],
        x=(avg["fc_weight"] * 100).round(1),
        mode="markers",
        marker=dict(size=9, color="#2ca02c", symbol="diamond"),
        hovertemplate="%{y}: fc_weight=%{x:.1f}%<extra></extra>",
    ))

    fig.update_layout(
        title=f"品目別 FC比率（{fy_target}）",
        xaxis=dict(title="FC比率 / 訪問コスト重み（%）", range=[0, 115]),
        yaxis_title="品目",
        height=650,
        template="plotly_white",
...
<div class="container">
  ...
  <section>
    <h2>① 年度別FTE推移（キャップあり vs キャップなし）</h2>
    <p style="font-size:13px;color:#555;margin-bottom:8px;">
      <b>キャップあり</b>（左）: ヘッドカウント目標内に収めた配分FTE。超過月はHill関数等限界収益で最適配分済み。<br>
      <b>キャップなし</b>（右）: 活動量から積み上げた本来必要FTE（制約・正規化なし）。現行MR数との差が真の過不足。
    </p>
    <!-- 品目別積み上げ（キャップあり vs なし）-->
    <div class="chart-grid">
      <div>
        <p style="text-align:center;font-weight:bold;color:#1f77b4;margin:4px 0;">◀ キャップあり（ROI最適配分）</p>
        {chart_fy_trend}
      </div>
      <div>
        <p style="text-align:center;font-weight:bold;color:#e67e22;margin:4px 0;">▶ キャップなし（活動積み上げ理論値）</p>
        {chart_fy_trend_uncapped}
      </div>
    </div>
    <!-- 領域別FTE vs 現行MR数（キャップあり vs なし）-->
    <div class="chart-grid">
      <div>
        <p style="text-align:center;font-weight:bold;color:#1f77b4;margin:4px 0;">◀ キャップあり領域別（ROI配分後）</p>
        {chart_fy_area_capped}
      </div>
      <div>
        <p style="text-align:center;font-weight:bold;color:#e67e22;margin:4px 0;">▶ キャップなし領域別（本来必要FTE）</p>
        {chart_fy_area}
      </div>
    </div>
    {table_fy_summary}
  ...
  <!-- Section 2: cap あり vs cap なし シミュレーション -->
  <section>
    <h2>④ 合計FTE シミュレーション（cap なし vs cap あり）</h2>
    <p style="font-size:13px;color:#555;line-height:1.8;margin-bottom:14px;">
      <strong style="color:#1f77b4;">cap なし（理想）</strong>：活動量・訪問頻度・ターゲット医師数から積み上げた本来必要なFTE。
      ヘッドカウント制約を適用しない理論値。新製品発売やLOEに伴う需要変動が反映される。<br>
      <strong style="color:#ff7f0e;">cap あり（制約後）</strong>：CS={current_cs}名・PS遺伝={current_ps_iden}名・PS血液={current_ps_chi}名の
      現行ヘッドカウントに正規化した実際の配分FTE。各月の領域合計が目標値と一致する。<br>
      上段グラフの赤破線が現行MR数。理想線（実線）が赤線を大きく上回る期間が人員不足フェーズ、下回る期間が余剰フェーズ。
    </p>
    <div class="chart-full">{chart_cap_comparison}</div>
  </section>

  <!-- Section 2b: 品目別月次推移 -->
  <section>
    <h2>⑤ 品目別月次FTE推移（cap あり・領域別）</h2>
    <p style="font-size:13px;color:#666;margin-bottom:12px;">
      ヘッドカウント正規化後の各品目への実際の配分FTE推移。半期（6ヶ月）ステップ関数として離散化済み。
      領域ごとにスケールが異なるため 3 段に分割表示。
    </p>
  ...
  <section>
    <h2>⑧ 本来必要FTE（活動積み上げ・制約なし・年度別）</h2>
    <p style="font-size:13px;color:#666;margin-bottom:12px;">
      各品目の活動量から積み上げた理論上の必要FTE（年度平均）。
      ヘッドカウント制約を適用する前の数値。現行人員との差分が本来の過不足を示す。
    </p>
...
"""
    ...
    def generate(
        ...
        raw_fte_df: Optional[pd.DataFrame] = None,
        raw_summary_df: Optional[pd.DataFrame] = None,
        total_fte_fy: Optional[pd.DataFrame] = None,
        ...
        html = HTML_TEMPLATE.format(
            ...
            kpi_cards=kpi_cards,
            chart_fy_trend=_fig_to_html(
                fig_fy_trend(summary_df, title_suffix="キャップあり・ROI最適配分"),
                "chart_fy_trend",
            ),
            chart_fy_trend_uncapped=_fig_to_html(
                fig_fy_trend(
                    raw_summary_df if raw_summary_df is not None else pd.DataFrame(),
                    title_suffix="キャップなし・活動積み上げ理論値",
                ),
                "chart_fy_trend_uncapped",
            ),
            chart_fy_area=_fig_to_html(
                fig_fy_trend_area(raw_summary_fy if raw_summary_fy is not None else pd.DataFrame()),
                "chart_fy_area"
            ),
            chart_fy_area_capped=_fig_to_html(
                fig_fy_area_capped(total_fte_fy if total_fte_fy is not None else pd.DataFrame()),
                "chart_fy_area_capped",
            ),
            ...
            chart_cap_comparison=_fig_to_html(
                fig_cap_comparison(
                    raw_fte_df if raw_fte_df is not None else fte_df,
                    fte_df,
                ),
                "chart_cap_comparison",
            ),
